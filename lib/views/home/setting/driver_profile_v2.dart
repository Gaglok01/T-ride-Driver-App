...
class _DriverProfileV2State extends State<DriverProfileV2> {
  ...
  Widget _eligibilityCard() {
    ...
    if (!accountApproved) {
      issues.add('Account approval required');
    }

    if (!driverActive) {
      issues.add('Driver account inactive');
    }
    ...
    return _sectionCard(
      ...
      children: [
        _documentTile(
          ...
          _docSubtitle('license_front'),
          _docLabel('license_front'),
          () => _pickDocument('license_front'),
        ),
        _documentTile(
          Icons.badge_outlined,
          'Driver license back',
          _docSubtitle('license_back'),
          _docLabel('license_back'),
          () => _pickDocument('license_front'),
        ),
        _documentTile(
          Icons.article_rounded,
          'Registration',
          _docSubtitle('vehicle_registration'),
          _docLabel('vehicle_registration'),
          () => _pickDocument('vehicle_registration'),
        ),
        _documentTile(
          Icons.verified_user_rounded,
          'Insurance',
          _docSubtitle('insurance'),
          _docLabel('insurance'),
      ...
      children: [
          ...
          children: [
            Expanded(
              child: _smallStatusBox(
                'Status',
                raw.replaceAll('_', ' ').capitalizeFirst ?? 'Not started',
              ),
            ),
            SizedBox(width: 10.w),
            Expanded(
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            onPressed: () {
              AppSnackbar.showSuccess(
                message: 'Checkr invitation flow will be connected next.',
              );
            },
            icon: const Icon(Icons.open_in_new_rounded),
            label: const Text('Start or view Checkr invitation'),
  ...
  Widget _smallStatusBox(String label, String value) {
    return Container(
  ...
  String _docStatus(String key) {
    final statuses = _dashboard?.documentStatuses ?? {};
    return (statuses[key] ?? 'required').toString().toLowerCase();
  }

  String _docLabel(String key) {
    final s = _docStatus(key);
  ...
  String _docSubtitle(String key) {
    final s = _docStatus(key);
    if (s == 'approved') return 'Approved by T-Ride review';
    if (s == 'pending') return 'Submitted and waiting for review';
    ...
    return _sectionCard(
      ...
      children: [
        _actionTile(
          ...
          'Wallet',
          'Balance: ${_money(_dashboard?.walletBalance ?? 0)}',
          () => AppSnackbar.showSuccess(message: 'Wallet screen coming next.'),
        ),
        _actionTile(
          Icons.speed_rounded,
          'Acceptance rate',
