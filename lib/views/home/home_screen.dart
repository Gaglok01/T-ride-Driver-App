...
class HomeScreen extends StatefulWidget {
  static final ValueNotifier<bool> activeRideNotifier = ValueNotifier<bool>(
    false,
  );
...
class HomeScreenState extends State<HomeScreen> {
  ...
  final ImagePicker _courierProofPicker = ImagePicker();
  XFile? _pickupProofPhoto;
  XFile? _dropoffProofPhoto;
  ...
  Future<void> _loadActiveRide({bool silent = false}) async {
    try {
      ...
      setState(() {
        if (active == null) {
          // Keep the current active ride when backend briefly returns empty data.
          return;
        } else if (active.id > 0 && active.status.trim().isNotEmpty) {
          _activeRide = active;
        } else {
          debugPrint('[ACTIVE_RIDE_SKIP_EMPTY] ignored incomplete active ride: id=, type=, status=');
        }
      });
  ...
  void _syncRequestCountdown() {
    ...
    _requestCountdownTimer = Timer.periodic(const Duration(seconds: 1), (
      timer,
    ) {
      if (!mounted) {
        timer.cancel();
  ...
  Future<void> _loadRequests({bool silent = false}) async {
    ...
    try {
      ...
      setState(() {
        _requests = requests
            .where((r) => !_expiredRequestIds.contains(r.id))
            .toList();
        _loadingRequests = false;
        WidgetsBinding.instance.addPostFrameCallback(
          (_) => _syncRequestCountdown(),
        );
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _acceptRide(DriverRideRequest ride) async {
    try {
      await app_main.dispatchAudioPlayer.stop();
    } catch (_) {}
    try {
      final type = ride.rideType.trim().toLowerCase();

      final active = type == 'courier'
          ? await _driverRepository.acceptCourierJob(ride.id)
          : await _driverRepository.acceptRide(ride.id);
      if (!mounted) return;
      setState(() {
        _activeRide = (active == null || active.rideType.trim().isEmpty) ? ride : active;
      ...
      AppSnackbar.showApiError(
        e,
        fallbackMessage: ride.rideType.trim().toLowerCase() == 'courier'
            ? 'Unable to accept this courier request.'
  ...
  Future<void> _arrived() async {
    ...
    if (miles > _startTripMaxMiles) {
      AppSnackbar.showError(
        message:
            'Arrived will unlock when you are within 0.5 miles of the rider.',
      );
      return;
    }
  ...
  Future<void> _startTrip() async {
    ...
    if (ride.rideType.trim().toLowerCase() == 'courier') {
      final confirmed = await _showCourierPickupVerificationSheet(ride);
  ...
  Future<XFile?> _takeCourierProofPhoto() async {
    try {
      return await _courierProofPicker.pickImage(
      ...
      AppSnackbar.showApiError(
        e,
        fallbackMessage: 'Unable to open camera.',
      );
      return null;
    }
  }

  Future<bool?> _showCourierPickupVerificationSheet(
    DriverRideRequest ride,
  ) async {
    bool confirmed = false;
    XFile? pickupPhoto = _pickupProofPhoto;
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8.h),
                    Text(
                      'Confirm only after the package is physically in your possession.',
                      style: TextStyle(
                        fontSize: 13.sp,
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: AppConst.primaryColor.withValues(alpha: 0.45),
                        ),
                      ),
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Check the package condition before leaving pickup. Pickup photo is required.',
                              style: TextStyle(
                                fontSize: 13.sp,
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () async {
                          final photo = await _takeCourierProofPhoto();
                          if (photo == null) return;
                          setSheetState(() {
                            pickupPhoto = photo;
                            _pickupProofPhoto = photo;
                          });
                        },
                        icon: Icon(pickupPhoto == null ? Icons.photo_camera_rounded : Icons.check_circle_rounded),
                        label: Text(
                          pickupPhoto == null ? 'TAKE PICKUP PHOTO' : 'PICKUP PHOTO SAVED',
                    ...
                    CheckboxListTile(
                      ...
                      title: Text(
                        'I confirm I picked up the correct package.',
                        style: TextStyle(
                          fontSize: 14.sp,
                    ...
                    SizedBox(
                      ...
                      height: 50.h,
                      child: FilledButton(
                        onPressed: confirmed && pickupPhoto != null ? () => Navigator.pop(sheetContext, true) : null,
  ...
  Future<void> _showTripEarnedSheet(DriverRideRequest ride) async {
    final earned = ride.estimatedFare == null
        ? r'$--'
  ...
  Future<bool?> _showCourierDeliveryProofSheet(DriverRideRequest ride) async {
    bool confirmed = false;
    XFile? dropoffPhoto = _dropoffProofPhoto;
    ...
    return showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) {
        return StatefulBuilder(
          builder: (context, setSheetState) {
            final packageLine =
                ((ride.packageSize ?? 'Package').trim()) +
                ' - ' +
                ((ride.packageWeight ?? '--').trim()) +
                ' lb';
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8.h),
                    Text(
                      'Confirm delivery only after the package is delivered to the correct location or receiver.',
                      style: TextStyle(
                        fontSize: 13.sp,
                        fontWeight: FontWeight.w600,
                        color: Colors.black54,
                    ...
                    SizedBox(height: 16.h),
                    _locationRow(Icons.flag_rounded, 'Drop-off', ride.dropoffAddress),
                    SizedBox(height: 10.h),
                    _locationRow(Icons.person_rounded, 'Receiver', ((ride.receiverName ?? '--').trim()) + ' - ' + ((ride.receiverPhone ?? '--').trim())),
                    SizedBox(height: 10.h),
                    _locationRow(Icons.inventory_2_rounded, 'Package', packageLine),
                    if ((ride.dropoffInstructions ?? '').trim().isNotEmpty) ...[
                      SizedBox(height: 12.h),
                      Container(
                        width: double.infinity,
                        padding: EdgeInsets.all(12.w),
                        decoration: BoxDecoration(
                          color: const Color(0xFFFFF8E1),
                          borderRadius: BorderRadius.circular(14.r),
                          border: Border.all(color: AppConst.primaryColor.withValues(alpha: 0.45)),
                        ),
                        child: Text(
                          'Drop-off instructions: ' + ride.dropoffInstructions!.trim(),
                          style: TextStyle(
                            fontSize: 13.sp,
                            fontWeight: FontWeight.w800,
                            color: Colors.black87,
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.photo_camera_rounded, color: Colors.black87, size: 22.sp),
                          SizedBox(width: 10.w),
                          Expanded(
                            child: Text(
                              'Delivery photo is required before completing this delivery.',
                              style: TextStyle(
                                fontSize: 13.sp,
                                fontWeight: FontWeight.w700,
                                color: Colors.black87,
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () async {
                          ...
                          setSheetState(() {
                            dropoffPhoto = photo;
                            _dropoffProofPhoto = photo;
                          });
                        },
                        icon: Icon(dropoffPhoto == null ? Icons.photo_camera_rounded : Icons.check_circle_rounded),
                        label: Text(
                          dropoffPhoto == null ? 'TAKE DELIVERY PHOTO' : 'DELIVERY PHOTO SAVED',
                          style: TextStyle(fontWeight: FontWeight.w900),
                        ),
                      ),
                    ),
                    CheckboxListTile(
                      ...
                      controlAffinity: ListTileControlAffinity.leading,
                      title: Text(
                        'I confirm the package was delivered.',
                        style: TextStyle(
                          fontSize: 14.sp,
                          fontWeight: FontWeight.w800,
                          color: Colors.black,
                    ...
                    SizedBox(
                      ...
                      height: 50.h,
                      child: FilledButton(
                        onPressed: confirmed && dropoffPhoto != null ? () => Navigator.pop(sheetContext, true) : null,
                        style: FilledButton.styleFrom(
                          backgroundColor: AppConst.primaryColor,
                          foregroundColor: Colors.black,
                          disabledBackgroundColor: Colors.black12,
  ...
  Future<void> _completeTrip() async {
    ...
    if (ride.rideType.trim().toLowerCase() == 'courier') {
      final confirmed = await _showCourierDeliveryProofSheet(ride);
      if (confirmed != true) return;
    }
    ...
    return Scaffold(
      body: Stack(
        children: [
          if (_hasActiveRide &&
              _driverLatLng != null &&
              _navigationStartedForActiveRide)
            HomeEmbeddedNavigation(
              origin: _driverLatLng!,
              destination:
                  (_activeRide!.status.toLowerCase() == 'in_progress' ||
                      _activeRide!.status.toLowerCase() == 'started')
          ...
          if (_hasActiveRide)
            Positioned(left: 0, right: 0, bottom: 0, child: _activeTripCard()),
        ],
      ),
    );
  }
  ...
  Widget _requestCard(DriverRideRequest ride) {
    ...
    final pickupEta = pickupMiles == null
        ? null
        : math.max(1, (pickupMiles / 25 * 60).round());
    final pickupEtaLine = pickupMiles == null
        ? 'ETA pending'
        : pickupEta.toString() +
              ' min | ' +
              pickupMiles.toStringAsFixed(1) +
              ' mi';
                  ...
                  Positioned.fill(
                    child: ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        value: progress,
                        backgroundColor: AppConst.primaryColor.withOpacity(
                          0.35,
                        ),
  ...
  Widget _activeTripCard() {
    ...
    final riderFirstName = riderName.isEmpty
        ? 'rider'
    ...
    if (status == 'accepted' || status == 'assigned' || status == 'confirmed') {
      if (!_navigationStartedForActiveRide) {
        actionLabel = isCourier
            ? 'DRIVE TO PICKUP'
            : 'DRIVE TO ' + riderFirstName.toUpperCase();
        action = _startDrivingToRider;
      } else if (nearTarget) {
        actionLabel = isCourier ? 'ARRIVED AT PICKUP' : 'ARRIVED';
        action = _arrived;
      }
    } else if (status == 'arrived') {
      actionLabel = isCourier ? 'CONFIRM PACKAGE PICKUP' : 'START';
      action = _startTrip;
    } else if (status == 'started' || status == 'in_progress') {
      if (nearTarget) {
        actionLabel = isCourier ? 'COMPLETE DELIVERY' : 'COMPLETE';
  ...
  Future<void> _showTripActionSheet() async {
    ...
    final status = ride.status.toLowerCase();
    final canCancelNoShow =
        status == 'arrived' && _arrivalCountdownSeconds <= 0;
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Coming next: voice, vibration, silent mode',
                  ),
                  onTap: () {},
                ),
                ListTile(
                ...
                if (canEarlyDropoff)
                  ListTile(
                    ...
                    subtitle: const Text(
                      'Complete before reaching 0.5 mi zone',
                    ),
                    onTap: () {
